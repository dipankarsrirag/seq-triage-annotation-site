...
export default function AnnotatePage() {
  ...
  useEffect(() => {
    ...
    const shuffled = seededShuffle(conversations, hashString(stored));
    setOrder(shuffled);

    fetchCompletedConversationIds(stored)
      .then((ids) => {
        setCompletedIds(ids);
    ...
    return (
      <div className="page">
        <div className="card">
          <h1>All done!</h1>
          <p>
            Thank you, {clinician}. You have completed all {order?.length ?? 0}{" "}
            conversations.
          </p>
        </div>
      </div>
